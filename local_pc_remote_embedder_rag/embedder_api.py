...
def _build_request(texts: list[str], config: ProjectConfig) -> request.Request:
    payload = {"model": config.embedding_api_model, "input": texts}
    ...
    return request.Request(url=config.embedding_api_url, data=body, headers=headers, method="POST")
...
def embed_texts(texts: list[str], config: ProjectConfig) -> list[list[float]]:
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            print(
                f"Embedding request {attempt}/{max_attempts}: {len(texts)} text(s) -> {config.embedding_api_url}"
            )
            req = _build_request(texts, config)
